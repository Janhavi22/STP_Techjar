...
const FlowmeterUpload = () => {
  ...
  const handleImageSelect = async (selectedFile: File) => {
    ...
    try {
      ...
      if (data.flow_value !== null) {
        setExtractedValue(data.flow_value);
        // Update readings immediately with the new value
        const now = new Date();
        const newReading = {
          id: readings.length + 1,
          date: now.toLocaleDateString(),
          time: now.toLocaleTimeString(),
          value: data.flow_value,
          site: user?.siteName || "Unknown",
          tot: data.flow_value,
          todays_tot: data.flow_value,
          image_url: data.image_url,
        };
        setReadings((prev) => [newReading, ...prev]);
      } else {
        toast.error("Could not extract reading from image");
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const valueToSubmit = manualValue
  ...
  useEffect(() => {
    if (user?.siteName) {
      fetch(`http://localhost:8000/meter-readings/${user.siteName}`)
        .then((res) => res.json())
        .then((data) => setReadings(data));
    }
  }, [user?.siteName, uploadSuccess]);

  return (
    <div className="mx-auto max-w-3xl p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">
          Upload Flowmeter Reading
        </h1>
        <p className="mt-1 text-slate-500">
          Upload a clear image of your flowmeter display for automatic reading
          extraction
        </p>
      </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-sm">
        ...
        <div className="mt-8">
          <h2 className="text-lg font-semibold mb-2">Meter Readings</h2>
            ...
            <table className="min-w-full border text-sm">
              <thead>
                <tr className="bg-slate-100">
                  ...
                  <th className="px-2 py-1 border">TOT</th>
                  <th className="px-2 py-1 border">SITE</th>
                  <th className="px-2 py-1 border">Today's TOT</th>
                  <th className="px-2 py-1 border">Image</th>
                </tr>
              </thead>
              <tbody>
                {readings.map((reading, idx) => {
                  const prevValue = idx > 0 ? readings[idx - 1].value : 0;
                  const tot = reading.value - prevValue;
                  return (
                    <tr key={reading.id}>
                      <td className="border px-2 py-1">{reading.id}</td>
                      <td className="border px-2 py-1">{reading.date}</td>
                      <td className="border px-2 py-1">{reading.time}</td>
                      <td className="border px-2 py-1">{tot}</td>
                      <td className="border px-2 py-1">{reading.site}</td>
                      <td className="border px-2 py-1">{reading.todays_tot}</td>
                      <td className="border px-2 py-1">
                        {reading.image_url && (
                          <img
                            src={`http://localhost:8000${reading.image_url}`}
            ...
            <div className="rounded-lg border border-success-200 bg-success-50 p-4">
              <div className="flex items-center text-success-800">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    The flowmeter reading has been recorded in the system.
                  </p>
                </div>
              </div>
            </div>
